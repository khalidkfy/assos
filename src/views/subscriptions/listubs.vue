<template>
          <container>
			<router-link class="btn w-25"  :to="{name:'addsubs', }">  اضافة اشتراك</router-link>
    ...
    <table class="table">
      ...
      <tbody v-for="informat in informats" :key="informat.id">
        <tr>
          ...
          <th scope="row">			<router-link class="btn "  :to="{name:'adds',query:{q:informat.id }}">  اضافة </router-link>
</th>
          <th scope="row">			<router-link class="btn "  :to="{name:'shwsubs',query:{q:informat.id }}">  عرض</router-link>
</th>
...
import container from '@/components/containers/container.vue';
  ...
  methods: {  
    ...
    basicdata() {
      ...
      axios
        .get(
          `api/subsc/index?subscription_type=${this.search}&start_subscription&end_subscription&code=1`, {
            headers: {
